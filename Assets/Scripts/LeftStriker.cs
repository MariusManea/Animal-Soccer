...
public class LeftStriker : MonoBehaviour {
	...
	void Update () {
		...
		if (GameObject.Find ("Bomber").GetComponent<Bomber> ().left_bombed == false && GameObject.Find ("Ball").GetComponent<Ball> ().left_broke == false) {
			z = this.transform.rotation.z;
			if (able)
			if (Input.GetKeyDown (master.GetComponent<Selected>().Keys[3]) && Time.timeScale == 1) {
				sources [0].Play ();
				able = false;
				strike = true;
			}
			...
			if (Mathf.Abs (z) < 0.5f)
				...
				if (Mathf.Abs (z) > 0.05f) {
					...
					if (kicked) {
						...
						GameObject.Find ("Ball").GetComponent<Rigidbody2D> ().AddForce (Vector2.right * x);
