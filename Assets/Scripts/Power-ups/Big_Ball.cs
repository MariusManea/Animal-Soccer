...
public class Big_Ball : MonoBehaviour {
	...
	void Update () {
		...
		if (triggered) {
			anim_writer.SetInteger ("Writing", 4);
			ball.GetComponent<Ball> ().big = true;
			if (ball.GetComponent<Ball> ().small)
				ball.GetComponent<Ball> ().small = false;
			ball.GetComponent<Ball> ().time_big = Time.time;
