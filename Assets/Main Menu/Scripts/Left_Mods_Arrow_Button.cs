...
public class Left_Mods_Arrow_Button : MonoBehaviour {
	...
	{ 
		...
		if (!GameObject.Find ("SettingsButton").GetComponent<SettingsButton> ().active) {
			if (mode != 1) {
				...
				GameObject.Find ("Mods_Arrow_Button2").GetComponent<Right_Mods_Arrow_Button> ().mode = mode - 1;
				mode = mode - 1;
			}
	...
	void Update()
	{
		if(mode==1) anim2.SetBool ("Blocked", true);
